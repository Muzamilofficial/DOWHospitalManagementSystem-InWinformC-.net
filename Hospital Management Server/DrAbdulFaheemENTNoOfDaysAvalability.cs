...
{
    public partial class DrAbdulFaheemENTNoOfDaysAvalability : Form
    {
        public DrAbdulFaheemENTNoOfDaysAvalability()
        {
            InitializeComponent();
        ...
        private void DrAbdulFaheemENTNoOfDaysAvalability_Load(object sender, EventArgs e)
        {
        ...
        public void DayAvailale()
        {
            if (comboBox1.Text != "")
            {
                ...
                SqlCommand cmd;
                string sql = "SELECT COUNT(Day) FROM DrAbdulFaheemBooking where Day='" + comboBox1.Text + "'";
                try
                {
                    mk.Open();
